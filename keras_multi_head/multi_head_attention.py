...
class MultiHeadAttention(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        def create_projection_model():
            inputs = keras.models.Input(shape = (self.input_size, self.linformer_depth))
            outputs = keras.layers.Dense(self.linformer_depth, name = 'proj', kernel_initializer = 'glorot_normal', bias_initializer = 'glorot_normal')(inputs)
            return keras.models.Model(inputs, outputs)

        projection_model = create_projection_model()
        
        def create_attention_model():
            queries = keras.models.Input(shape = (self.input_size, self.linformer_depth))
            keys = keras.models.Input(shape = (self.input_size, self.linformer_depth))
            values = keras.models.Input(shape = (self.input_size, self.linformer_depth))
            causal_mask = keras.models.Input(shape = (self.input_size, self.linformer_depth))
            infinity_mask = keras.models.Input(shape = (self.input_size, self.linformer_depth))
            ...
            p_bar = keras.layers.Lambda(lambda x: x * K.constant(1 / np.sqrt(self.linformer_depth)), name = 'p_bar', output_shape = lambda s: (s))(qk) # 0, 1024, 64
        ...
        queries = keras.layers.Dense(self.linformer_depth)(inputs)
        keys = keras.layers.Dense(self.linformer_depth)(inputs)
        values = keras.layers.Dense(self.linformer_depth)(inputs)
        causal_mask = keras.layers.Lambda(lambda x: K.constant(np.expand_dims(np.transpose(np.triu(np.ones([self.linformer_depth, self.input_size]), 0)), 0)), output_shape = lambda s: (None, s[1], s[2]), name = 'causal_mask')(queries) # 0, 64, 1024 #only once compute
